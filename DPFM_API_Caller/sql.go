...
func (c *DPFMAPICaller) headerCreateSql(
	...
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": headerData, "function": "KanbanContainerHeader", "runtime_session_id": sessionID})
	if err != nil {
...
func headerIsUpdate(header *dpfm_api_processing_formatter.HeaderUpdates) bool {
	kanbanContainer := header.KanbanContainer

	return !(kanbanContainer == 0)
}
